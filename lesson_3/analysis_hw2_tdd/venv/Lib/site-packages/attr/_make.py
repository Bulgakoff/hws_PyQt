...
class _CacheHashWrapper(int):
    ...
    This is used for non-slots classes_for_hw5 with ``cache_hash=True``, to avoid
...
_Attributes = _make_attr_tuple_class(
    ...
    [
        ...
        "base_attrs",
        # map inherited attributes to their originating classes_for_hw5
        "base_attrs_map",
    ],
)
...
def _is_class_var(annot):
    ...
    annotations which would put attrs-based classes_for_hw5 at a performance
    disadvantage compared to plain old classes_for_hw5.
    """
...
def _collect_base_attrs(cls, taken_attr_names):
    """
    Collect attr.ibs from base classes_for_hw5 of *cls*, except *taken_attr_names*.
    """
    base_attrs = []
    base_attr_map = {}  # A dictionary of base attrs to their classes_for_hw5.
...
if PYPY:
    ...
    def _frozen_setattrs(self, name, value):
        """
        Attached to frozen classes_for_hw5 as __setattr__.
        """
...
def _frozen_delattrs(self, name):
    """
    Attached to frozen classes_for_hw5 as __delattr__.
    """
    raise FrozenInstanceError()
...
class _ClassBuilder(object):
    ...
    def _create_slots_class(self):
        ...
        # MRO because we only care about our immediate base classes_for_hw5.
...
def attrs(
    ...
    :param str repr_ns: When using nested classes_for_hw5, there's no way in Python 2
    ...
    :param bool auto_detect: Instead of setting the *init*, *repr*, *eq*,
        *order*, and *hash* arguments explicitly, assume they are set to
        ...
        ``__eq__`` *nor* ``__ne__`` (but Python classes_for_hw5 come with a sensible
    ...
    :param Optional[bool] eq: If ``True`` or ``None`` (default), add ``__eq__``
        ...
        attributes if and only if the types of both classes_for_hw5 are *identical*!
    ...
    :param bool slots: Create a `slotted class <slotted classes_for_hw5>` that's more
        memory-efficient. Slotted classes_for_hw5 are generally superior to the default
        dict classes_for_hw5, but have some gotchas you should know about, so we
        encourage you to read the `glossary entry <slotted classes_for_hw5>`.
    ...
    :param bool auto_exc: If the class subclasses `BaseException`
        ...
        - all attributes that are either passed into ``__init__`` or have a
          ...
          attribute,
        - the value of *str* is ignored leaving ``__str__`` to base classes_for_hw5.
    :param bool collect_by_mro: Setting this to `True` fixes the way ``attrs``
       collects attributes from base classes_for_hw5.  The default behavior is
    ...
    :param Optional[bool] getstate_setstate:
       .. note::
          This is usually only interesting for slotted classes_for_hw5 and you should
          probably just set *auto_detect* to `True`.

       If `True`, ``__getstate__`` and
       ``__setstate__`` are generated and attached to the class. This is
       necessary for slotted classes_for_hw5 to be pickleable. If left `None`, it's
       `True` by default for slotted classes_for_hw5 and ``False`` for dict classes_for_hw5.
    ...
    .. deprecated:: 18.2.0
       ...
       `DeprecationWarning` if the classes_for_hw5 compared are subclasses of
    ...
    def wrap(cls):
        ...
        if getattr(cls, "__class__", None) is None:
            raise TypeError("attrs only works with new-style classes_for_hw5.")
...
def _make_init(
    ...
    if frozen and has_global_on_setattr:
        raise ValueError("Frozen classes_for_hw5 can't use on_setattr.")
...
def _attrs_to_init_script(
    ...
    if frozen is True:
        if slots is True:
            ...
            # Dict frozen classes_for_hw5 assign directly to __dict__.
